...
export async function POST(
  ...
  const current = db
    .prepare("SELECT id, position FROM insights WHERE id = ?")
  ...
  if (current) {
    const neighbor = db
      .prepare(
        direction === "up"
          ? "SELECT id, position FROM insights WHERE position < ? ORDER BY position DESC LIMIT 1"
          : "SELECT id, position FROM insights WHERE position > ? ORDER BY position ASC LIMIT 1"
      )
      .get(current.position) as { id: number; position: number } | undefined;

    if (neighbor) {
      const update = db.prepare("UPDATE insights SET position = ? WHERE id = ?");
