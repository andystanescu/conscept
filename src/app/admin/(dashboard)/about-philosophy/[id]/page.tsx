...
export default async function EditPhilosophyItemPage({
  params,
  searchParams,
}: {
  ...
  const item = db
    .prepare("SELECT * FROM about_philosophy_items WHERE id = ?")
  ...
  return (
    <>
      <h1 className="heading-01">Edit philosophy item</h1>
      {error && <p style={{ color: "var(--border-error)" }}>{error}</p>}
      <form
        className={styles.form}
        action={`/api/admin/about-philosophy/${item.id}`}
        method="POST"
