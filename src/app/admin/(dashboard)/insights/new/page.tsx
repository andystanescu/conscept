...
export default async function NewInsightPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <><h1 className="heading-01">New article</h1>{error && <p style={{ color: "var(--border-error)" }}>{error}</p>}<InsightEditor action="/api/admin/insights" categories={categories} settingsAuthor={getSettings().author_name} /></>;
}
